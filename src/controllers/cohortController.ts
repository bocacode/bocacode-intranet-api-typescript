...
export const disableCohort: RequestHandler = async (req, res) => {
  ...
  try {
    const { id } = req.params
    const cohortUpdated = await Cohorts.findOneAndUpdate({ uid: id }, { $set: { enabled: false } }, { new: true })
    ...
    if (cohortUpdated) {
      ...
      addLog(log)
      res.status(200).send({ success: `Cohort id ${id} has been disabled` })
    } else {
      res.status(404).send({ error: 'No cohort found with the provided id' })
    }
  } catch (err) {
    res.status(500).send({ error: err })
  }
